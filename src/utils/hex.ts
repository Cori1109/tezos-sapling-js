...
export function toHexStringRaw(
  value: number | BigNumber,
  bitLength: number = 8
): string {
...
function toHexStringRawPositive(
  value: number | BigNumber,
  bitLength: number
): string {
  ...
  let targetLength: number =
    hexString.length >= nibbleLength ? hexString.length : nibbleLength
  targetLength = targetLength % 2 == 0 ? targetLength : targetLength + 1

  return padStart(hexString, targetLength, "0")
}

function toHexStringRawNegative(
  value: number | BigNumber,
  bitLength: number
): string {
  const value2sComplement: BigNumber = new BigNumber(2)
    .pow(bitLength)
    .minus(new BigNumber(value).abs())
