...
import {
  ...
  TezosWrappedTransactionOperation,
...
export abstract class TezosFAProtocol
  extends TezosProtocol
  implements ICoinSubProtocol
{
  ...
  public async getTransactionsFromAddresses(
    ...
    const lastEntryBlockLevel: number =
      allTransactions.length > 0
        ? allTransactions[allTransactions.length - 1].block_level
        : 0

    const transactions = allTransactions
      .reduce((current, next) => current.concat(next))
      .map((transaction: any) =>
        this.transactionToAirGapTransactions(transaction, addresses)
      )
      .reduce(
        (flatten: IAirGapTransaction[], toFlatten: IAirGapTransaction[]) =>
          flatten.concat(toFlatten),
        []
      )
  ...
  public async getTransactions(
    ...
    const transactions: IAirGapTransaction[] = response.data
      .map((transaction: any) =>
        this.transactionToAirGapTransactions(transaction)
      )
      .reduce(
        (flatten: IAirGapTransaction[], toFlatten: IAirGapTransaction[]) =>
          flatten.concat(toFlatten),
    ...
    const lastEntryBlockLevel: number =
      response.data.length > 0
        ? response.data[response.data.length - 1].block_level
        : 0
  ...
  public async normalizeTransactionParameters(
    ...
    if (
      !(parsedParameters instanceof Object && "value" in parsedParameters) &&
      !isMichelineNode(parsedParameters)
    ) {
      throw new InvalidValueError(
        Domain.TEZOSFA,
        `Invalid parameters: ${JSON.stringify(parsedParameters)}`
      )
    }

    return this.contract.normalizeContractCallParameters(
      parsedParameters,
      fallbackEntrypointName
    )
  }
  ...
  protected async getTransactionOperationDetails(
    ...
    try {
      partials = transactionOperation.parameters
        ? this.transactionDetailsFromParameters(
            transactionOperation.parameters
          ) ?? []
        : []
    } catch {}
  ...
  private transactionToAirGapTransactions(
    ...
    const parameters: string =
      transaction.parameters_micheline ?? transaction.parameters
    const parsedParameters: unknown = this.parseParameters(parameters)

    if (!isMichelineNode(parsedParameters)) {
      throw new InvalidValueError(
        Domain.TEZOSFA,
        `Transaction parameters are invalid: ${JSON.stringify(
          parsedParameters
        )}`
      )
    }
    ...
    const partialDetails: Partial<IAirGapTransaction>[] =
      this.transactionDetailsFromParameters(transferData)

    return partialDetails.map((details: Partial<IAirGapTransaction>) => {
      const inbound: boolean =
        sourceAddresses !== undefined && details.to && details.to.length === 1
  ...
  protected async runContractCall(
    contractCall: TezosContractCall,
    source: string
  ): Promise<MichelineDataNode> {
    const results: AxiosResponse[] = await Promise.all([
      axios.get(
        this.url(`/chains/main/blocks/head/context/contracts/${source}/counter`)
      ),
    ...
    const body = this.prepareMockContractCall(
      contractCall,
      chainID,
      branch,
      counter,
    ...
    try {
      const response = await axios.post(
        this.url("/chains/main/blocks/head/helpers/scripts/run_operation"),
        body,
        {
      ...
      if (
        metadata.internal_operation_results !== undefined &&
  ...
  protected async prepareContractCall(
    ...
    const operations: TezosOperation[] = contractCalls.map(
      (contractCall: TezosContractCall) => {
        return {
    ...
    try {
      const tezosWrappedOperation: TezosWrappedOperation =
        await this.prepareOperations(publicKey, operations, false)
  ...
  protected requireSource(
    ...
    return _source === undefined ||
      excludedPrefixes.some((excluded: string) =>
        _source.toLowerCase().startsWith(excluded.toLowerCase())
      )
  ...
  private prepareMockContractCall(
    ...
    return {
      ...
      operation: {
        branch,
        signature:
          "sigUHx32f9wesZ1n2BWpixXz4AQaZggEtchaQNHYGRCoWNAXx45WGW2ua3apUUUAGMLPwAU41QoaFCzVSL61VaessLg4YbbP", // signature will not be checked, so it is ok to always use this one
        contents: [
          {
            kind: TezosOperationType.TRANSACTION,
