...
export class TezosBTC extends TezosFA12Protocol {
  private static readonly bigMapKeyLedgerPrefix: string =
    "0x05070701000000066c65646765720a00000016"
  ...
  public async getTotalMinted(
    source?: string,
    callbackContract: string = this.callbackContract()
  ): Promise<string> {
    const getTotalMintedCall = await this.contract.createContractCall(
      TezosBTCContractEntrypoint.TOTAL_MINTED,
      [[], callbackContract]
    )
  ...
  public async getTotalBurned(
    source?: string,
    callbackContract: string = this.callbackContract()
  ): Promise<string> {
    const getTotalBurnedCall = await this.contract.createContractCall(
      TezosBTCContractEntrypoint.TOTAL_BURNED,
      [[], callbackContract]
    )
  ...
  > {
    ...
    return values
      .map((bigMapEntry) => {
        const addressHex = bigMapEntry.key.substring(
          TezosBTC.bigMapKeyLedgerPrefix.length
        )
        ...
        const amount: BigNumber =
          value instanceof MichelsonInt ? value.value : new BigNumber(0)
