...
export class TezosFA2Protocol extends TezosFAProtocol {
  ...
  private readonly defaultCallbackContract: Partial<
    Record<TezosNetwork, Partial<Record<TezosFA2ContractEntrypoint, string>>>
  >
  ...
  constructor(options: TezosFA2ProtocolOptions) {
    ...
    this.tokenMetadataBigMapName =
      options.config.tokenMetadataBigMapName ??
      TezosFA2Protocol.DEFAULT_TOKEN_METADATA_BIG_MAP_NAME
    this.tokenMedatadaBigMapValueRegex =
      options.config.tokenMetadataBigMapRegex ??
      TezosFA2Protocol.DEFAULT_TOKEN_METADATA_BIG_MAP_VALUE_REGEX

    this.defaultCallbackContract = {
      [TezosNetwork.MAINNET]: {
        [TezosFA2ContractEntrypoint.BALANCE]:
          "KT1LyHDYnML5eCuTEVCTynUpivwG6ns6khiG",
        [TezosFA2ContractEntrypoint.TOKEN_METADATA_REGISTRY]:
          "KT1L16VBmW8tkovhLmonhfvf6dtTZya6k6af",
      },
      [TezosNetwork.EDONET]: {
        [TezosFA2ContractEntrypoint.BALANCE]:
          "KT1UEyZsmSga2KcNkpGbWX6nvGAVHtBpT5is",
        [TezosFA2ContractEntrypoint.TOKEN_METADATA_REGISTRY]:
          "KT1MMhwbCMfwzDUu4ur3SEpaf6njn2nsMDBt",
      },
    }
  }
  ...
  public async getBalanceOfAddresses(addresses: string[]): Promise<string> {
    ...
    if (tokenID === undefined) {
      throw new InvalidValueError(
        Domain.TEZOS,
        "Can't check the balance, no tokenID has been provided."
      )
    }
    ...
    return results
      .reduce(
        (sum: BigNumber, next: TezosFA2BalanceOfResponse) =>
          sum.plus(next.amount),
        new BigNumber(0)
      )
      .toFixed()
  }
  ...
  public async estimateFeeDefaultsFromPublicKey(
    ...
    const transferCall: TezosContractCall = await this.createTransferCall(
      publicKey,
      recipients,
      values,
                  ...
                  MichelsonPair.from(
                    ...
                    (pairJSON: string) =>
                      MichelsonPair.from(
                        ...
                        undefined,
                        (tokenJSON: string) =>
                          MichelsonInt.from(tokenJSON, "token_id"),
                        (amountJSON: string) =>
                          MichelsonInt.from(amountJSON, "amount")
                      )
                  ),
                "txs"
              )
      ...
      return Array.isArray(callArgumentsList)
        ? callArgumentsList
            .map((callArguments: unknown) => {
              ...
              const from: string = isHex(callArguments.from_)
                ? TezosUtils.parseAddress(callArguments.from_)
                : callArguments.from_

              const transferDetails: [string, BigNumber, BigNumber][] =
                callArguments.txs.map((tx) => {
                  const to: string = isHex(tx.to_)
                    ? TezosUtils.parseAddress(tx.to_)
                    : tx.to_

                  return [to, tx.token_id, tx.amount] as [
                    string,
                    BigNumber,
                    BigNumber
                  ]
              ...
              return transferDetails.map(
                ([to, tokenID, amount]: [string, BigNumber, BigNumber]) => {
                  return {
                    ...defaultDetails,
                    ...
                    extra: {
                      type: parameters.entrypoint,
                      assetID:
                        this.tokenID === undefined || !tokenID.eq(this.tokenID)
                          ? tokenID.toString()
                          : undefined,
                    },
                  }
            ...
            .reduce(
              (
                flatten: Partial<IAirGapTransaction>[],
                next: Partial<IAirGapTransaction>[]
              ) => flatten.concat(next),
              []
            )
        : [defaultDetails]
  ...
  public async balanceOf(
    ...
    const balanceOfCall: TezosContractCall =
    ...
    const results: MichelineDataNode = await this.runContractCall(
      balanceOfCall,
      ...
      return results
        .map((node: MichelineDataNode) => {
          try {
            ...
            const accountWithTokenID: MichelsonPair = MichelsonPair.from(
              pair.items[0].get()
            )
            const account: MichelsonAddress = MichelsonAddress.from(
              accountWithTokenID.items[0].get()
            )
            const tokenID: MichelsonInt = MichelsonInt.from(
              accountWithTokenID.items[1].get()
            )
            ...
            return {
              address:
                account.address instanceof MichelsonString
                  ? account.address.value
                  : TezosUtils.parseAddress(account.address.value),
        ...
        .filter(
          (balanceOfResults: TezosFA2BalanceOfResponse | undefined) =>
            balanceOfResults !== undefined
        ) as TezosFA2BalanceOfResponse[]
    } else {
      return []
    }
  }
  ...
  public async transfer(
    ...
    const transferCall: TezosContractCall =
      await this.contract.createContractCall(
  ...
  public async updateOperators(
    ...
    const updateCall: TezosContractCall =
      await this.contract.createContractCall(
  ...
  public async tokenMetadataRegistry(
    ...
    const tokenMetadataRegistryCall: TezosContractCall =
      await this.contract.createContractCall(
        TezosFA2ContractEntrypoint.TOKEN_METADATA_REGISTRY,
        callbackContract
      )

    const result: MichelineDataNode = await this.runContractCall(
      tokenMetadataRegistryCall,
      this.requireSource(source)
    ).catch(() => [])
  ...
  public async tokenMetadata(
    ...
    const tokenMetadataCall: TezosContractCall =
      await this.contract.createContractCall(
  ...
  public async getTokenMetadata(
    ...
    return metadataResponse
      .map((response: BigMapResponse) => {
        const match = response.value
          ? this.tokenMedatadaBigMapValueRegex.exec(response.value)
          : null
      ...
      .filter(
        (metadata: TezosFA2TokenMetadata | null) => metadata !== null
      ) as TezosFA2TokenMetadata[]
  }

  public async fetchTokenHolders(
    tokenIDs: number[]
  ): Promise<{ address: string; amount: string; tokenID: number }[]> {
  ...
  private async createTransferCall(
    ...
    const tokenID: number =
      data && data.tokenID !== undefined ? data.tokenID : 0
    ...
    const recipientsWithValues: [string, string][] = recipients.map(
      (recipient: string, index: number) => [recipient, values[index]]
    )
    ...
    const transferCall: TezosContractCall =
      await this.contract.createContractCall(
        ...
        [
          {
            from_: fromAddress,
            txs: recipientsWithValues.map(
              ([recipient, value]: [string, string]) => {
                return {
                  to_: recipient,
                  token_id: tokenID,
                  amount: value,
  ...
  protected callbackContract(entrypoint: TezosFA2ContractEntrypoint): string {
    const networkCallbacks:
      | Partial<Record<TezosFA2ContractEntrypoint, string>>
      | undefined =
      this.defaultCallbackContract[this.options.network.extras.network]
    const callback: string | undefined = networkCallbacks
      ? networkCallbacks[entrypoint]
      : undefined
  ...
  private isTransferRequest(obj: unknown): obj is {
    ...
    return (
      ...
      anyObj.txs.every(
        (tx: any) =>
          tx instanceof Object &&
          typeof tx.to_ === "string" &&
          BigNumber.isBigNumber(tx.token_id) &&
          BigNumber.isBigNumber(tx.amount)
      )
    )
  }
