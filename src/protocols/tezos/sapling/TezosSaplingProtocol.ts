...
} from "../types/operations/Transaction"
...
export abstract class TezosSaplingProtocol
  ...
  implements ICoinProtocol
{
  ...
  public readonly addressValidationPattern: string =
    "^(zet1[1-9A-Za-z]{65}|tz1[1-9A-Za-z]{33})$"
  ...
  constructor(public readonly options: TezosSaplingProtocolOptions) {
    ...
    this.tezosProtocol = new TezosProtocol(
      new TezosProtocolOptions(this.options.network, new TezosProtocolConfig())
    )

    this.name = options.config.name
    this.symbol = options.config.symbol ?? this.tezosProtocol.symbol
    this.marketSymbol =
      options.config.marketSymbol ?? this.tezosProtocol.marketSymbol

    this.feeSymbol = this.tezosProtocol.feeSymbol
    this.feeDefaults =
      options.config.feeDefaults ?? this.tezosProtocol.feeDefaults
    ...
    this.nodeClient = new TezosSaplingNodeClient(
    ...
    this.forger = new TezosSaplingForger(
      this.cryptoClient,
      this.state,
      this.encoder,
      this.options.config.externalProvider
    )
    this.bookkeeper = new TezosSaplingBookkeeper(
      this.identifier,
  ...
  public async initParameters(
    ...
    const externalInitParameters =
      this.options.config.externalProvider?.initParameters
  ...
  public async getPublicKeyFromHexSecret(
    ...
    const viewingKey: Buffer = await sapling.getExtendedFullViewingKey(
      secret,
      derivationPath
    )
  ...
  public async getAddressFromViewingKey(
    viewingKey: string,
    index: string
  ): Promise<TezosSaplingAddress> {
  ...
  public async getTransactionsFromPublicKey(
    ...
    const commitmentsAndCiphertexts: [
      string,
      ...
      .map(
        (
          ...
          [commitment, ciphertext, new BigNumber(index)] as [
            string,
            TezosSaplingCiphertext,
            BigNumber
          ]
    ...
    const [incoming, outgoing]: [TezosSaplingInput[], TezosSaplingInput[]] =
      await Promise.all([
  ...
  public async signWithPrivateKey(
    ...
    const stateTree: TezosSaplingStateTree =
      await this.state.getStateTreeFromStateDiff(transaction.stateDiff)

    const forgedTransaction: TezosSaplingTransaction =
      await this.forger.forgeSaplingTransaction(
        transaction.ins,
        transaction.outs,
        stateTree,
    ...
    const signed: string = this.encoder
      .encodeTransaction(forgedTransaction)
      .toString("hex")

    return transaction.callParameters.replace(
      TezosSaplingProtocol.TRANSACTION_PLACEHOLDER,
      signed
    )
  }
  ...
  public async getTransactionDetails(
    ...
    if (this.isRawTezosSaplingTransaction(tx)) {
      ...
      const parameters: TezosTransactionParameters =
        this.contract.parseParameters(tx.callParameters)
      const wrappedTransactions: TezosSaplingWrappedTransaction[] =
        await this.parseParameters(parameters)

      airGapTxs.push(
        ...this.bookkeeper.getUnsignedTransactionDetails(
          from,
          tx.ins,
          tx.outs,
          wrappedTransactions
      ...
      const wrappedOperation: TezosWrappedOperation =
        await this.tezosProtocol.unforgeUnsignedTezosWrappedOperation(
          tx.binaryTransaction
        )

      airGapTxs.push(
        ...(await this.getTransactionDetailsFromWrappedOperation(
          wrappedOperation,
  ...
  public async getTransactionDetailsFromSigned(
    ...
    if (this.contract.areValidParameters(transaction.transaction)) {
      const partialDetails: Partial<IAirGapTransaction>[] =
        await this.getPartialDetailsFromContractParameters(
      ...
      const wrappedOperation: TezosWrappedOperation =
        await this.tezosProtocol.unforgeSignedTezosWrappedOperation(
          transaction.transaction
        )
  ...
  private async getTransactionDetailsFromWrappedOperation(
    ...
    type TezosContractOperation = TezosTransactionOperation &
      Required<Pick<TezosTransactionOperation, "parameters">>
    const contractOperations: TezosContractOperation[] =
      operation.contents.filter(
        (content: TezosOperation) =>
          content.kind === TezosOperationType.TRANSACTION &&
          (content as TezosTransactionOperation).parameters !== undefined
    ...
    const details: IAirGapTransaction[][] = await Promise.all(
      contractOperations.map(async (operation: TezosContractOperation) => {
        const saplingDetails: Partial<IAirGapTransaction>[] =
          await this.getPartialDetailsFromContractParameters(
  ...
  private async getPartialDetailsFromContractParameters(
    ...
    const parameters: TezosTransactionParameters =
      typeof rawOrParsed === "string"
        ? this.contract.parseParameters(rawOrParsed)
        : rawOrParsed

    const wrappedTransactions: TezosSaplingWrappedTransaction[] =
      await this.parseParameters(parameters)

    return this.bookkeeper.getWrappedTransactionsPartialDetails(
      wrappedTransactions,
      knownViewingKeys
    )
  }
  ...
  private filterOutPaybacks(
    ...
    const filtered: IAirGapTransaction[] = airGapTxs.filter(
      (details: IAirGapTransaction) => {
        ...
        return Array.from(fromSet).some(
          (address: string) => !toSet.has(address)
        )
      }
    )
  ...
  public async prepareTransactionFromPublicKey(
    ...
    return Promise.reject(
      `Invalid recpient, expected a 'tz' or 'zet' address, got ${recipient}`
    )
  }
  ...
  public async wrapSaplingTransactions(
    ...
    if (typeof transactions === "string") {
      ...
      operations = [
        this.prepareTezosOperation(parameters, fee) as TezosOperation,
      ]
    } else {
      const contractCalls: TezosContractCall[] =
        await this.prepareContractCalls(transactions)
      operations = contractCalls.map(
        (contractCall: TezosContractCall) =>
          this.prepareTezosOperation(
            contractCall.toJSON(),
            fee,
            contractCall.amount?.toString()
          ) as TezosOperation
      )
    }
    ...
    try {
      const tezosWrappedOperation: TezosWrappedOperation =
        await this.tezosProtocol.prepareOperations(
          publicKey,
          operations,
          overrideFees
        )
      const binaryTx: string = await this.tezosProtocol.forgeTezosOperation(
  ...
  public async prepareShieldTransaction(
    ...
    const stateTree: TezosSaplingStateTree =
      await this.state.getStateTreeFromStateDiff(saplingStateDiff, true)
    ...
    const encodedTransaction: string = this.encoder
      .encodeTransaction(forgedTransaction)
      .toString("hex")
    const wrappedTransaction: TezosSaplingWrappedTransaction = {
      signed: encodedTransaction,
    }

    return this.wrapSaplingTransactions(
      publicKey,
      [wrappedTransaction],
      fee,
      data?.overrideFees
  ...
  public async prepareUnshieldTransaction(
    ...
    const contractCall: TezosContractCall =
      await this.preparePartialContractCall(recipient)
  ...
  public async prepareSaplingTransaction(
    ...
    if (!TezosSaplingAddress.isZetAddress(recipient)) {
      return Promise.reject(
        `Invalid recpient, expected a 'zet' address, got ${recipient}`
      )
    }

    const dummyInputsAmount: number = Math.max(
      new BigNumber(data?.dummyInputs ?? 0).toNumber(),
      0
    )
    const dummyOutputsAmount: number = Math.max(
      new BigNumber(data?.dummyOutputs ?? 0).toNumber(),
      0
    )
    ...
    const address: TezosSaplingAddress = await this.getAddressFromPublicKey(
      viewingKey
    )
    const paybackOutput: TezosSaplingOutput = {
    ...
    const contractCall: TezosContractCall =
      await this.preparePartialContractCall()

    const [dummyInputs, dummyOutputs]: [
      TezosSaplingInput[],
      TezosSaplingOutput[]
    ] = await Promise.all([
      Promise.all(
        Array.from({ length: dummyInputsAmount }, (_v, _k) =>
          this.createDummyInput(address)
        )
      ),
      Promise.all(
        Array.from({ length: dummyOutputsAmount }, (_v, _k) =>
  ...
  private async preparePartialContractCall(
    ...
    const wrappedTransaction: TezosSaplingWrappedTransaction = {
      ...
      unshieldTarget:
        unshieldTarget !== undefined
          ? await TezosAddress.fromValue(unshieldTarget)
          : undefined,
    }
    const contractCalls: TezosContractCall[] = await this.prepareContractCalls([
      wrappedTransaction,
    ])
  ...
  private async createDummyInput(
  ...
  private async createDummyOutput(): Promise<TezosSaplingOutput> {
    ...
    const memo: Uint8Array = sodium.randombytes_buf(
      this.options.config.memoSize
    )
  ...
  private async getDummyAddress(): Promise<TezosSaplingAddress> {
    ...
    const viewingKey: string = await this.getPublicKeyFromHexSecret(
  ...
  public async getBalanceOfPublicKeyForSubProtocols(
    ...
    throw new Error(
      "Method `getBalanceOfPublicKeyForSubProtocols` not supported."
    )
  }
  ...
  private isRawTezosSaplingTransaction(
    transaction: unknown
  ): transaction is RawTezosSaplingTransaction {
    return (
