...
export class TezosSaplingCryptoClient extends Ed25519CryptoClient {
  ...
  public async encryptCiphertext(
    ...
    const address: TezosSaplingAddress = await TezosSaplingAddress.fromValue(
      tezosOutput.address
    )
    const [ovk, diversifier, pkd]: [Buffer | undefined, Buffer, Buffer] =
      await Promise.all([
        viewingKey !== undefined
          ? sapling.getOutgoingViewingKey(viewingKey)
          : undefined,
    ...
    const payloadEnc: Uint8Array = sodium.crypto_secretbox_easy(
      plaintextEnc,
      nonceEnc,
      symKey
    )
    ...
    const payloadOut: Uint8Array = sodium.crypto_secretbox_easy(
      plaintextOut,
      nonceOut,
      ock
    )
  ...
  public async decryptCiphertextEnc(
    ...
    const amount: BigNumber = new BigNumber(
      decrypted.slice(11, 19).toString("hex"),
      16
    )
  ...
  public async decryptCiphertextOut(
    ...
    const cm: Buffer = Buffer.isBuffer(commitment)
      ? commitment
      : Buffer.from(commitment, "hex")
    const epk: Buffer = Buffer.from(ciphertext.epk, "hex")
    const ock: Uint8Array = blake2bAsBytes(
      Buffer.concat([cv, cm, epk, ovk]),
      256,
      {
        key: Buffer.from(this.ockKey),
      }
  ...
  private async getSymmetricKey(
    p: Buffer | Uint8Array | string,
    sk: Buffer | Uint8Array | string
  ): Promise<Buffer> {
    const ka: Buffer = await sapling.keyAgreement(p, sk)

    return Buffer.from(
      blake2bAsBytes(ka, 256, { key: Buffer.from(this.kdfKey) })
    )
  }
}
