...
export class TezosSaplingAddress extends TezosAddress {
  private constructor(
    ...
    public readonly diversifierIndex?: string
  ) {
    super(value)
  }

  public static async fromViewingKey(
    viewingKey: string,
    index?: Buffer | string | number
  ): Promise<TezosSaplingAddress> {
    const paymentAddress: SaplingPaymentAddress =
      await getPaymentAddressFromViewingKey(viewingKey, index)
  ...
  public static async next(
    ...
    if (current.diversifierIndex === undefined) {
      return Promise.reject(
        `Can't get next address for undefined diversifier index`
      )
    }

    const nextAddress: SaplingPaymentAddress =
      await getNextPaymentAddressFromViewingKey(
        viewingKey,
