...
export class TezosSaplingBookkeeper {
  ...
  public getUnsignedTransactionDetails(
    ...
    const outputsDetails: IAirGapTransaction[] = outputs.map(
      (out: TezosSaplingOutput) => ({
    ...
    const unshieldDetails: IAirGapTransaction[] = wrappedTransactions
      .map((wrappedTransaction: TezosSaplingWrappedTransaction) => {
        ...
        const amount: BigNumber = this.sumNotes(inputs).minus(
          this.sumNotes(outputs)
        )
      ...
      .filter(
        (details: IAirGapTransaction | undefined) => details !== undefined
  ...
  public async getWrappedTransactionsPartialDetails(
    ...
    const partials: Partial<IAirGapTransaction>[][] = await Promise.all(
      wrappedTransactions.map(
        async (wrappedTransaction: TezosSaplingWrappedTransaction) => {
          const transaction: TezosSaplingTransaction =
            this.encoder.decodeTransaction(
              Buffer.from(wrappedTransaction.signed, "hex")
            )
          const [from, details]: [
            string | undefined,
            Partial<IAirGapTransaction>[]
          ] = await this.getTransactionPartialDetails(
            transaction,
            knownViewingKeys
          )
          ...
          if (wrappedTransaction.unshieldTarget !== undefined) {
            ...
            if (from !== undefined) {
              unshieldDetails = Object.assign(unshieldDetails, {
                from: [
                  (await TezosSaplingAddress.fromViewingKey(from)).getValue(),
                ],
              })
            }
  ...
  private async getTransactionPartialDetails(
    ...
    const sender: string | undefined =
      transaction.spendDescriptions.length === 0
        ? undefined
        : await this.findSender(transaction, knownViewingKeys)

    const details: Partial<IAirGapTransaction>[] = await Promise.all(
      transaction.outputDescriptions.map(
        async (description: TezosSaplingOutputDescription) => {
          const recipient: string | undefined = await this.findRecipient(
            description,
            knownViewingKeys
          )

          const from: TezosSaplingAddress | undefined =
            sender !== undefined
              ? await TezosSaplingAddress.fromViewingKey(sender)
              : undefined
          const [to, amount]: [
            TezosSaplingAddress | undefined,
            BigNumber | undefined
          ] = await this.decodeDetailsFromOutputDescription(
            sender,
            recipient,
            description
          )
          ...
          if (from !== undefined) {
            outputDetails = Object.assign(outputDetails, {
              from: [from.getValue()],
            })
          }
          if (to !== undefined) {
      ...
      await Promise.all(
        viewingKeys.map(async (viewingKey: string) => {
          try {
            ...
            const outputDescription: TezosSaplingOutputDescription =
              transaction.outputDescriptions[0]
            await this.cryptoClient.decryptCiphertextOut(
              viewingKey,
              outputDescription.ciphertext,
              outputDescription.cm
            )
  ...
  private async findRecipient(
    outputDescription: TezosSaplingOutputDescription,
    viewingKeys: string[]
  ): Promise<string | undefined> {
    return (
      await Promise.all(
  ...
  public sumNotes(
    notes: (TezosSaplingInput | TezosSaplingOutput)[]
  ): BigNumber {
    return notes.reduce(
      (sum: BigNumber, next: TezosSaplingInput | TezosSaplingOutput) =>
        sum.plus(next.value),
      new BigNumber(0)
    )
  }
    ...
    const inputs: TezosSaplingInput[] = (
      await Promise.all(
        commitmentsWithCiphertext.map(
          async ([commitment, ciphertext, position]: [
            string,
            TezosSaplingCiphertext,
            BigNumber
          ]) => {
            const decrypted = await this.getIncomingInputFromCiphertext(
              viewingKey,
              ciphertext,
              commitment,
              position
      ...
      (input: TezosSaplingInput | undefined) => input !== undefined
    ) as TezosSaplingInput[]
      ...
      await Promise.all(
        commitmentsWithCiphertext.map(
          async ([commitment, ciphertext, position]: [
            ...
            const decrypted = await this.getOutgoingInputFromCiphertext(
              viewingKey,
              ciphertext,
              commitment,
              position
  ...
  public async getUnspends(
    ...
    const nullifiersSet: Set<string> = new Set(
      nullifiers.map((nullifier: string) => stripHexPrefix(nullifier))
    )

    const inputs: TezosSaplingInput[] = await this.getInputs(
      viewingKey,
    ...
    const unspends: TezosSaplingInput[] = (
      await Promise.all(
        inputs.map(async (input: TezosSaplingInput) => {
          ...
          return !nullifiersSet.has(nullifier.toString("hex"))
            ? input
            : undefined
        })
      )
  ...
  private async getReceiverInputFromCiphertext(
    ...
    try {
      ...
      const address: Buffer =
        await sapling.getRawPaymentAddressFromIncomingViewingKey(
          ivk,
          diversifier
        )
  ...
  private async getIncomingInputFromCiphertext(
    ...
    const inputWithMemo: [Buffer, TezosSaplingInput] | undefined =
      await this.getReceiverInputFromCiphertext(
        viewingKey,
        ciphertext,
        position
  ...
  private async getOutgoingInputFromCiphertext(
    ...
    const inputWithMemo: [Buffer, TezosSaplingInput] | undefined =
      await this.getSenderInputFromCiphertext(
        viewingKey,
        ciphertext,
        commitment,
        position
