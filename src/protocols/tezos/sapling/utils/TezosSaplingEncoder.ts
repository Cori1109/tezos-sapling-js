...
export class TezosSaplingEncoder {
  public encodeTransaction(transaction: TezosSaplingTransaction): Buffer {
    const spendBytes: Buffer = this.encodeSpendDescriptions(
      transaction.spendDescriptions
    )
    const spendEncoded: Buffer = Buffer.concat([
      toHexBuffer(spendBytes.length, 32),
      spendBytes,
    ])

    const outBytes: Buffer = this.encodeOutputDescriptions(
      transaction.outputDescriptions
    )
    const outEncoded: Buffer = Buffer.concat([
      toHexBuffer(outBytes.length, 32),
      outBytes,
    ])
    ...
    return Buffer.concat([
      spendEncoded,
      outEncoded,
      transaction.bindingSignature,
      balanceEncoded,
      Buffer.from(transaction.root, "hex"),
    ])
  }

  public encodeSpendDescriptions(
    descriptions: TezosSaplingSpendDescription[]
  ): Buffer {
    return this.encodeDescriptions(
      descriptions,
      this.encodeSpendDescription.bind(this)
    )
  }

  public encodeSpendDescription(
    description: TezosSaplingSpendDescription
  ): Buffer {
    return Buffer.concat([
      description.cv,
      description.nf,
  ...
  public encodeOutputDescriptions(
    descriptions: TezosSaplingOutputDescription[]
  ): Buffer {
    return this.encodeDescriptions(
      descriptions,
      this.encodeOutputDescription.bind(this)
    )
  }

  public encodeOutputDescription(
    description: TezosSaplingOutputDescription
  ): Buffer {
    const ciphertextBytes: Buffer = this.encodeCiphertext(
  ...
  private encodeDescriptions<T>(
    descriptions: T[],
    encoder: (description: T) => Buffer
  ): Buffer {
  ...
  private encodeCiphertext(ciphertext: TezosSaplingCiphertext): Buffer {
    ...
    return Buffer.concat([
      cv,
      epk,
      toHexBuffer(payloadEnc.length, 32),
      payloadEnc,
      nonceEnc,
      payloadOut,
      nonceOut,
  ...
  public decodeTransaction(transaction: Buffer): TezosSaplingTransaction {
    ...
    const spendDescriptions: TezosSaplingSpendDescription[] =
      this.decodeSpendDescriptions(transaction.slice(spendStart, spendEnd))
    ...
    const outputDescriptions: TezosSaplingOutputDescription[] =
      this.decodeOutputDescriptions(transaction.slice(outStart, outEnd))
    ...
    const balance: BigNumber = this.decodeBalance(
      transaction.slice(balanceStart, balanceEnd)
    )
  ...
  public decodeSpendDescriptions(
    bytes: Buffer
  ): TezosSaplingSpendDescription[] {
    return this.decodeDescriptions(
      bytes,
      this.decodeSpendDescription.bind(this),
      this.encodeSpendDescription.bind(this)
    )
  }
  ...
  public decodeOutputDescriptions(
    bytes: Buffer
  ): TezosSaplingOutputDescription[] {
    return this.decodeDescriptions(
      bytes,
      this.decodeOutputDescription.bind(this),
      this.encodeOutputDescription.bind(this)
    )
  }

  private decodeDescriptions<T>(
    bytes: Buffer,
    decoder: (bytes: Buffer) => T,
    encoder: (description: T) => Buffer
  ): T[] {
  ...
  private decodeBalance(encoded: Buffer): BigNumber {
    ...
    return isPositive
      ? bigNumber
      : new BigNumber(2).pow(64).minus(bigNumber).negated()
  }
}
