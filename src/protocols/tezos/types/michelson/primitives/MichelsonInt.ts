...
export class MichelsonInt extends MichelsonType {
  ...
  public static fromMicheline(
    micheline: MichelinePrimitive<"int">,
    name?: string
  ): MichelsonInt {
  ...
  public static fromUnknown(
    ...
    if (
      ...
      !BigNumber.isBigNumber(unknownValue)
    ) {
      throw invalidArgumentTypeError(
        "MichelsonInt",
        "number or string or BigNumber",
        typeof unknownValue
      )
    }

    return new MichelsonInt(
      BigNumber.isBigNumber(unknownValue)
        ? unknownValue
        : new BigNumber(unknownValue),
      name
    )
  }
