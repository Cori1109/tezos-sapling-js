...
export class TezosRewardsCalculationDefault
  implements TezosRewardsCalculations
{
  ...
  protected async getConstants(currentLevel: number, useHead: boolean) {
    this.tezosNodeConstants = (
      await axios.get(
        `${this.protocol.jsonRPCAPI}/chains/main/blocks/${
          useHead ? "head" : currentLevel
        }/context/constants`
      )
  ...
  public async calculateRewards(
    ...
    const currentCycle =
      currentCycleIn ?? (await this.protocol.fetchCurrentCycle())
    ...
    if (cycle < currentCycle) {
      computedRewards = await this.calculatePastRewards(
        bakerAddress,
        cycle,
        breakdownRewards
      )
    } else {
      computedRewards = await this.calculateFutureRewards(
        bakerAddress,
        cycle,
        currentCycle,
        breakdownRewards
    ...
    const snapshotLevel = await this.computeSnapshotBlockLevel(
      Math.min(cycle, currentCycle)
    )
    const bakerInfo = await this.fetchBakerInfo(
      bakerAddress,
      snapshotLevel
    ).catch(() => {
  ...
  private async calculatePastRewards(
    ...
    if (breakdownRewards) {
      ...
      if (bakedBlocks.length > 0) {
        computedBakingRewards = await this.computeBakingRewards(
          bakedBlocks,
          false
        )
      }

      const endorsingOperations = await this.fetchEndorsementOperations(
        cycle,
        bakerAddress
      )
      computedEndorsingRewards = await this.computeEndorsingRewards(
        endorsingOperations,
        false
      )
      endorsingRightsCount = endorsingOperations.reduce(
        (current, next) => current + next.number_of_slots,
        0
      )
    }

    const frozenBalance = (
      await this.fetchFrozenBalances(
        this.protocol.cycleToBlockLevel(cycle + 1),
        bakerAddress
      )
    ).find((fb) => fb.cycle == cycle)
  ...
  private async calculateFutureRewards(
    ...
    if (cycle - currentCycle > 5) {
      throw new InvalidValueError(
        Domain.TEZOS,
        `Provided cycle ${cycle} is invalid`
      )
    }
    ...
    if (breakdownRewards) {
      ...
      endorsingRightsCount = endorsingRights.reduce(
        (current, next) => current + next.number_of_slots,
        0
      )
    }
  ...
  private async fetchEndorsementOperations(
    cycle: number,
    bakerAddress: string
  ): Promise<TezosEndorsingRight[]> {
    const cycleStartLevel = this.protocol.cycleToBlockLevel(cycle)
    const cycleEndLevel =
      cycleStartLevel + this.tezosNodeConstants.blocks_per_cycle
    const query = {
    ...
    const result = await axios.post(
      `${this.protocol.baseApiUrl}/v2/data/tezos/${this.protocol.baseApiNetwork}/operations`,
      query,
      {
  ...
  private async fetchFrozenBalances(
    blockLevel: number | "head",
    bakerAddress: string
  ): Promise<TezosFrozenBalance[]> {
  ...
  protected async computeBakingRewards(
    ...
    result = new BigNumber(
      bakingRights.length *
        ((this.tezosNodeConstants as TezosNodeConstantsV1)
          .block_reward as unknown as number)
    )

    return {
      totalBakingRewards: result.toFixed(),
      rewardsDetails: rewardsByLevel,
    }
  }

  // be aware this function IS NOT identical to the one in TezosRewardCalculation005
  protected async computeEndorsingRewards(
    endorsingRights: TezosEndorsingRight[],
    isFutureCycle: boolean
  ): Promise<TezosEndorsingRewards> {
    ...
    const result = endorsingRights.reduce((current, next) => {
      ...
      if (!isFutureCycle) {
        ...
        if (block === undefined) {
          throw new InvalidValueError(
            Domain.TEZOS,
            "Cannot find block priority"
          )
        }
        priority = block.priority
      }

      const reward: BigNumber = this.specificEndorsingCalculation(
        priority,
    ...
    return {
      totalEndorsingRewards: result.toFixed(),
      rewardsDetails: rewardsByLevel,
    }
  }
  ...
  protected specificEndorsingCalculation(
    ...
    const multiplier = new BigNumber(
      (this.tezosNodeConstants as TezosNodeConstantsV1).endorsement_reward
    ).div(new BigNumber(priority + 1))
    const reward: BigNumber = new BigNumber(number_of_slots).times(multiplier)

    return reward
  }

  private async computeSnapshotBlockLevel(
    cycle: number,
    blockLevel?: number | "head"
  ): Promise<number> {
    const level =
      blockLevel === undefined
        ? this.protocol.cycleToBlockLevel(cycle)
        : blockLevel
    ...
    const firstDelegationCycleBlocklLevel =
      this.protocol.cycleToBlockLevel(delegationCycle)
    const numberOfSnapshotsBeforeDelegationCycle =
      firstDelegationCycleBlocklLevel /
      this.tezosNodeConstants.blocks_per_roll_snapshot
    const totalSnapshotNumber =
      numberOfSnapshotsBeforeDelegationCycle + snapshotNumber + 1
    const snapshotBlockLevel =
      totalSnapshotNumber * this.tezosNodeConstants.blocks_per_roll_snapshot

    return snapshotBlockLevel
  }

  private async fetchBakerInfo(
    bakerAddress: string,
    blockLevel: number | "head"
  ): Promise<TezosBakerInfo> {
  ...
  private async fetchBlocksForBaker(
    bakerAddress: string,
    cycle: number
  ): Promise<{ level: number; priority: number }[]> {
    const query = {
    ...
    const result = await axios.post(
      `${this.protocol.baseApiUrl}/v2/data/tezos/${this.protocol.baseApiNetwork}/blocks`,
      query,
      {
        headers: this.protocol.headers,
      }
  ...
  private readonly blockLevelFieldNameMap = new Map<
    "baking_rights" | "endorsing_rights",
    string
  >()
  private fetchBlockLevelFieldPromise?: Promise<string>

  private async fetchBlockLevelFieldName(
    entity: "baking_rights" | "endorsing_rights"
  ): Promise<string> {
    ...
    this.fetchBlockLevelFieldPromise = axios
      .get(
        `${this.protocol.baseApiUrl}/v2/metadata/tezos/${this.protocol.baseApiNetwork}/${entity}/attributes`,
        {
          headers: this.protocol.headers,
        }
      )
      .then((response) => {
        ...
        const fieldNames: string[] = response.data.map(
          (metadata: any) => metadata.name
        )
  ...
  protected async fetchBakingRights(
    ...
    const blockLevelFieldName = await this.fetchBlockLevelFieldName(
      "baking_rights"
    )
    const query = {
    ...
    const result = await axios
      .post(
        `${this.protocol.baseApiUrl}/v2/data/tezos/${this.protocol.baseApiNetwork}/baking_rights`,
        query,
        {
          headers: this.protocol.headers,
        }
    ...
    return result.map((bakingRight: any) => {
      return {
  ...
  protected async fetchEndorsingRights(
    bakerAddress: string,
    cycle: number
  ): Promise<TezosEndorsingRight[]> {
    const startLevel = this.protocol.cycleToBlockLevel(cycle)
    ...
    const result = await axios
      .post(
        `${this.protocol.baseApiUrl}/v2/data/tezos/${this.protocol.baseApiNetwork}/endorsing_rights`,
        query,
        {
          headers: this.protocol.headers,
        }
    ...
    return result.map((endorsingRight: any) => {
      return {
  ...
  protected async fetchEndorsementOperationCountAndTotalFees(
    ...
    Map<
      number,
      { sum_number_of_slots: string; block_level: number; sum_fee: number }
    >
  > {
    const query = {
      fields: ["block_level"],
  ...
  protected async fetchBlockPriorities(
    blockLevels: number[]
  ): Promise<{ priority: number; level: number }[]> {
    const query = {
