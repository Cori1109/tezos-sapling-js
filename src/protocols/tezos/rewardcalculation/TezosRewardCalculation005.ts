...
export class TezosRewardsCalculation005 extends TezosRewardsCalculationDefault {
  ...
  protected async computeBakingRewards(
    ...
    let endorsementCountsAndFees:
      ...
      | undefined
    if (!isFutureCycle) {
      endorsementCountsAndFees =
        await this.fetchEndorsementOperationCountAndTotalFees(levels)
    }
    result = bakingRights.reduce(
      (current: BigNumber, next: { level: number; priority: number }) => {
        // (16 / (priority + 1)) * (0.8 + (0.2 * (e / 32)))
        let count = this.tezosNodeConstants.endorsers_per_block
        let blockEndorsementCountAndFee:
          | {
        ...
        if (!isFutureCycle && endorsementCountsAndFees !== undefined) {
          ...
          if (blockEndorsementCountAndFee === undefined) {
            throw new InvalidValueError(
              Domain.TEZOS,
              "Cannot find endorsement operation count"
            )
          }
        ...
        rewardsByLevel.push({
          ...
          fees:
            blockEndorsementCountAndFee !== undefined
              ? new BigNumber(blockEndorsementCountAndFee.sum_fee).toFixed()
              : "0",
        })
