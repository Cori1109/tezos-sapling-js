...
import {
  ...
  MichelineTypeNode,
  MichelineDataNode,
} from "../types/micheline/MichelineNode"
...
export class TezosContract {
  ...
  public async bigMapValues(
    request: BigMapRequest = {}
  ): Promise<BigMapResponse[]> {
    const bigMapID: number =
      request?.bigMapID ?? (await this.getBigMapID(request.bigMapFilter))
  ...
  public async createContractCall(
    ...
    const entrypoint: TezosContractEntrypoint | undefined =
      this.entrypoints?.get(entrypointName)
    if (!entrypoint) {
      return this.createDefaultContractCall(value, amount)
    }

    return this.createEntrypointContractCall(
      entrypoint,
      value,
      undefined,
      amount
  ...
  public async parseContractCall(
    json: TezosTransactionParameters
  ): Promise<TezosContractCall> {
    await this.waitForEntrypoints()

    const entrypoint: TezosContractEntrypoint | undefined =
      this.entrypoints?.get(json.entrypoint)
    if (!entrypoint) {
      return Promise.reject(
        `Couldn't parse the contract call, unknown entrypoint: ${json.entrypoint}`
      )
    }
    ...
    return this.createEntrypointContractCall(entrypoint, json.value, {
      ...
      onNext: (
        ...
        const argName: string | undefined = meta.getAnnotation(
  ...
  public async normalizeContractCallParameters(
    json:
      | (Partial<TezosTransactionParameters> &
          Pick<TezosTransactionParameters, "value">)
      | MichelineNode,
    fallbackEntrypoint?: string
  ): Promise<TezosTransactionParameters> {
    const entrypoint: string | undefined =
      "entrypoint" in json ? json.entrypoint : undefined
    ...
    const defaultEntrypoint: TezosContractEntrypoint | undefined =
      this.entrypoints?.get(TezosContract.DEFAULT_ENTRYPOINT)
    if (!defaultEntrypoint) {
      throw new InvalidValueError(
        Domain.TEZOS,
        "Could not fetch default entrypoint."
      )
    }
    ...
    defaultEntrypoint.type.createValue(value, {
      beforeNext: (meta: MichelsonTypeMeta, raw: unknown): void => {
        const entrypointName: string | undefined = meta.getAnnotation(
          MichelsonAnnotationPrefix.FIELD
        )
    ...
    return {
      entrypoint: normalizedEntrypoint
        ? normalizedEntrypoint[0]
        : fallbackEntrypoint ?? defaultEntrypoint.name,
  ...
  private createDefaultContractCall(
    ...
    return new TezosContractCall(
      TezosContract.DEFAULT_ENTRYPOINT,
      value instanceof MichelsonType ? value : undefined,
      amount
    )
  }
  ...
  private createEntrypointContractCall(
    ...
    return new TezosContractCall(
      entrypoint.name,
      entrypoint.type.createValue(value, configuration),
      amount
    )
  }
  ...
  private async getBigMapID(predicates?: BigMapPredicate[]): Promise<number> {
    ...
    if (!predicates) {
      throw new InvalidValueError(
        Domain.TEZOS,
        "Contract has more than one BigMap, provide ID or predicates to select one."
      )
    }

    const response = await this.apiRequest<Record<"big_map_id", number>[]>(
      "big_maps",
      {
        fields: ["big_map_id"],
        predicates: [
    ...
    if (response.length > 1) {
      throw new InvalidValueError(
        Domain.TEZOS,
        "More than one BigMap ID has been found for the predicates."
      )
    }
  ...
  private async waitForBigMapIDs(): Promise<void> {
    ...
    if (this.bigMapIDsPromise === undefined) {
      this.bigMapIDsPromise = this.apiRequest<Record<"big_map_id", number>[]>(
  ...
  private async waitForEntrypoints(): Promise<void> {
    ...
    if (this.entrypointsPromise === undefined) {
      const codePromise: Promise<Record<"code", TezosContractCode[]>> =
        this.nodeRequest("script")
      const entrypointsPromise: Promise<
        Record<"entrypoints", Record<string, MichelineTypeNode>>
      > = this.nodeRequest("entrypoints")
      ...
      this.entrypointsPromise = Promise.all([codePromise, entrypointsPromise])
        .then(([codeResponse, entrypointsResponse]) => {
          if (
            ...
            const parameter = codeResponse.code.find(
              (primitiveApplication) =>
                primitiveApplication.prim === "parameter"
            )
            if (parameter) {
              entrypointsResponse.entrypoints[
                TezosContract.DEFAULT_ENTRYPOINT
              ] = parameter.args ? parameter.args[0] : []
            }
          }

          this.entrypoints = new Map(
            TezosContractEntrypoint.fromJSON(
              entrypointsResponse.entrypoints
            ).map((entrypoint: TezosContractEntrypoint) => [
              entrypoint.name,
              entrypoint,
            ])
          )
